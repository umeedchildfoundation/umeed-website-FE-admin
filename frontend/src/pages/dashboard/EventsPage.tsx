...
export default function EventsPage() {
  ...
  const BUCKET = (import.meta as any).env?.VITE_SUPABASE_EVENT_BUCKET || "event-media";
    ...
    mutationFn: async () => {
      ...
      if (selectedFile) {
        if (isDemoMode) {
          ...
          const uploadRes = await api.storage.from(BUCKET).upload(path, selectedFile, { upsert: true });
          if (uploadRes.error) {
