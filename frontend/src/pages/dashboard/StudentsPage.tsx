...
import { Plus, Users, CheckCircle2, User } from "lucide-react";
...
interface Student extends Omit<StudentFormData, "image_url" | "notes" | "location_code" | "roll_number"> {
  id: string;
...
export default function StudentsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: async (student: StudentFormData) => {
      ...
      const insertData: Record<string, any> = {
        ...
        area_locality: student.area_locality || null,
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await api.from("students").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["students"] });
