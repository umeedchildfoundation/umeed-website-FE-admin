...
export function AdminDashboard() {
    ...
    const futureSessions = sessionsWithCounts?.filter(s => new Date(s.session_date) >= today) || [];
    const nextSession = futureSessions.length > 0
        ? futureSessions.sort((a, b) => new Date(a.session_date).getTime() - new Date(b.session_date).getTime())[0]
        : null;
            ...
            <Card className="shadow-md">
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-2">
                        {sessionsWithCounts && sessionsWithCounts.map((session) => {
