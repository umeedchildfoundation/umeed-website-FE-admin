...
import { User, MapPin, Phone, School, FileText, Calendar, Clock, UserCheck, BookOpen, Users, Activity, Pencil, Trash2 } from "lucide-react";
...
export function StudentProfileSheet({ student, isOpen, onClose, onEdit }: StudentProfileSheetProps) {
    ...
    const handleDelete = async () => {
        try {
            const { error } = await api.from('students').delete().eq('id', student.id);
...
function AcademicFamilyTab({ student }: { student: Student }) {
    return (
        <div className="space-y-6">
            {/* Family Info */}
            <Card>
    ...
    const { data: history, isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (assignments?.length) {
                ...
                volunteers?.forEach(v => volNameMap[v.id] = v.name);
